...
class FullAttention(nn.Module):
    def __init__(self, mask_flag=True, factor=5, enc_in=7, sigma=2., scale=None, attention_dropout=0.1, output_attention=False):
        super(FullAttention, self).__init__()
        self.scale = scale
        self.mask_flag = mask_flag
        self.output_attention = output_attention
        self.dropout = nn.Dropout(attention_dropout)
        self.sigma = nn.Parameter(torch.tensor([sigma]*enc_in, dtype=torch.float32), requires_grad=True)
        
    def compute_position_diff_matrix(self, L, periods, device):
        position = torch.arange(L, device=device)
        position_diff_scales = []
        # print(periods.shape)
        for period in periods:
            position_diff = torch.abs(position[:, None] - position[None, :]) % period
            position_diff = torch.minimum(position_diff, period - position_diff)
            position_diff_scales.append(position_diff)
        return position_diff_scales
    
    def gaussian_decay(self, position_diff, sigma):
        sigma_expanded = sigma.unsqueeze(0).unsqueeze(2)
        decay_weights = torch.exp(-0.5 * (position_diff / sigma_expanded) ** 2)
        return decay_weights

    def forward(self, queries, keys, values, x_mark_enc, periods, attn_mask, tau=None, delta=None):
        B, L, H, E = queries.shape
        _, S, _, D = values.shape
        scale = self.scale or 1. / sqrt(E)

        scores = torch.einsum("blhe,bshe->bhls", queries, keys) # 32,8,11,11
        
        M = L if x_mark_enc is None else L - 4
        # print(M)
        position_diff_scales_tensor = torch.stack(self.compute_position_diff_matrix(M, periods, queries.device)) # 32,7,7
        decay_weights_scales_tensor = self.gaussian_decay(position_diff_scales_tensor, self.sigma)  # 32,7,7
        
        decay_weights = decay_weights_scales_tensor.unsqueeze(1)  #(32, 1, 7, 7)
        decay_weights = decay_weights.expand(-1, H, -1, -1)  #(32, 8, 7, 7)
        # print(decay_weights.shape)
        
        scores[:, :, :M, :M] = scores[:, :, :M, :M].clone() * decay_weights
        ...
        attention_matrix = A.detach().cpu().numpy()  # A
        folder_path = '/root/series_forecast/PAformer/save_att/'
        if not os.path.exists(folder_path):
            os.makedirs(folder_path)
        np.save(folder_path + 'ettm2.npy', attention_matrix)
